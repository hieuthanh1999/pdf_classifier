...
@time_execution
def classifier_credit_mtu_invoice(pages):
    try:
        page_data = {}
        for p_idx, page in enumerate(pages):
            ...
            for i, line in enumerate(text):
                ...
                if "INVOICE NO" in line and "DATE" in line:

                    invoice_number_match = re.search(r'INVOICE NO\.\s*:\s*([A-Za-z0-9\.]+)', line)
                    date_match = re.search(r'DATE:\s*(\d{1,2}/[A-Za-z]{3}/\d{2})', line)
                    if invoice_number_match:
                        invoice_number = invoice_number_match.group(1)
                        page_data['invoice_no'] = invoice_number
                    else:
                    ...
                    if date_match:
                        ...
                        page_data['date'] = ''
                if "Actual to credit" in line:
                    match = re.search(r'(\d{1,3}(?:,\d{3})*(?:\.\d{2}))\s*(\w+)', line)
                    if match:
                        amount = match.group(1)
                        page_data['actual_to_credit'] = to_float(amount)
